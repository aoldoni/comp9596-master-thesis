...
\begin{figure}[!htbp]
  ...
  \caption[Samples of the most common words in the singular third-person form.]{Samples of the most common words in the singular third-person form.}
...
\begin{figure}[!htbp]
  ...
  \caption[Samples of the most common words in the singular third-person form, after removing the top 2 words.]{Samples of the most common words in the singular third-person form, after removing the top 2 words.}
  \label{fig:words_most_use_no_top2}
\end{figure}

One of the initial attempts to explore how to extract information from the generated corpus was to use the Stanford Named Entity Recognizer (NER) to recognize the concepts discussed so far in the academic text. To do so, a small set of around 20 papers' \emph{Related Work} section was annotated for the concepts contained in them using Brat \cite{Stenetorp:2012:BWT:2380921.2380942}. An example of this annotated data can be seen in \hl{BRAT IMAGE REFERENCE}.

The model was trained mostly with the recommended settings and features.

The success was moderated with the NER model recognizing some clearly delineated concepts by its shape (.e.g: capital words), but for non-capitalized words it appeared as it would only recognize the concepts if its words were present in the training set.

\hl{STANFORD NER EXAMPLE TABLE}

\hl{- Attempt of using Stanford NER}

\hl{- Attempt of using Stanford NER with Gazetter}

\hl{- Attempt of using Stanford REL}
