...
An interesting example to start with is the sentence \emph{\enquote{We stress that our method improves a supervised baseline}}. The tree generated by the dependency parser is already \enquote{optimal} in the sense that the information is ready to be extracted without any tree manipulation. \hl{See Figure X for this tree}, the extracted relation would be \texttt{improves ( our method ; supervised baseline )}. We list now the rules created for our system to increase its ability to extract information in other \enquote{non-optimal} trees. 
...
An example for Rule \ref{treerule:rule4} is the partial sentence \emph{\enquote{matrix co-factorization helps to handle multiple aspects of the data and improves in predicting individual decisions}}, when searching for the \emph{\enquote{improves}} relation. While normally the parser would annotate \emph{\enquote{in predicting (...)}} as a sub clause with a \emph{prep} edge relation, this clause does contain the object being improved by the subject.
...
Another rule that does tree manipulation, Rule \ref{treerule:rule5} caters for situations where the relation being searched for is itself found in a \emph{subj}-like edge connected with its head node.
...
We observed that, in several situations, the subject or object sentences were too long, mainly due to containing too much information beyond the subject/object concept definition. With information extraction, it is reasonable to assume that the tool should return the information as granular as possible, while still maintaining the possibility for the user to use extra context if needed. In an attempt to alleviate this situation, Rule \ref{treerule:rule_subj4} was created.
...
After continuous revision, some rules were adjusted to, by omission, also cater for a certain number of cases such as:
	\begin{itemize}
...
\hl{Add table each rule and its python name}
...
Moreover, it was also possible to note the incapacity of the rules to be applied together, or chained, as to output the correct answers.

\hl{IS RULE employs	( subj: database system ; obj: serial transaction execution ) Sentence: H-Store is an example of a database system which employs serial transaction execution, and works best when workloads are perfectly partitionable [44].}

Moreover, a further challenge would be having the tool to be capable of deciding when to chain rules, or when the result is already optimal.

\hl{AND RULE Sentence: 37	employs	( subj: PAS analysis ; obj: global and sentence-wide features ) ( mod: widely ) As PAS analysis widely employs global and sentence-wide features, it is computationally expensive to integrate target side predicateargument structures into the dynamic programming style of SMT decoding (Wu and Fung, 2009b).}
